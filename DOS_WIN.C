...
void f_prn_proc()
{
 float V_tmp;
          ...
          {
            {
              ...
              if( flag_AD != 0)
              {
                ...
                //now:
                MmiGotoxy(0,9 ); MmiPrintf("Темп.средняя  %7.2f C       ", (Temp_sum_YN/s_MVD[0].MassT) );
                //23.01.2020 YN  -----//\\-----

                MmiGotoxy(0,10); MmiPuts( str_empt );
                MmiGotoxy(0,11); MmiPuts( str_empt );
                MmiGotoxy(0,12); MmiPrintf("Давление 1    %8.3f МПа     ",s_MVD[0].Press);
                //27.01.20 YN  -----\\//-----
                MmiGotoxy(0,13); MmiPrintf("Давление 2    %8.3f МПа     ",Press_N2);
                //27.01.20 YN  -----//\\-----
              }
              else
#endif
          ...
          {
                MmiGotoxy(0,7);  MmiPuts("-------Средневзвешенная-------");
                MmiGotoxy(0,8);  MmiPuts("Плотн,кг/м3| Темп,С  |Давл,МПа");

                if( flag_AD != 0)
                {
                  MmiGotoxy(0,9);
                  MmiPrintf(" %8.2f  |%8.2f |%8.3f ",(Dens_sum_YN/s_MVD[0].MassT),(Temp_sum_YN/s_MVD[0].MassT),(Press_sum_YN/s_MVD[0].MassT));
                  MmiGotoxy(0,10);  MmiPuts("------------------------------");
                }
                else 
                {
                  MmiGotoxy(0,9);
                  MmiPrintf(" %8.2f  |%8.2f |%8.3f ",s_MVD[0].Dens,s_MVD[0].Temp,s_MVD[0].Press);
                  MmiGotoxy(0,10);  MmiPuts("------------------------------");
                }
          }
          //23.01.2020 YN  -----//\\-----
...
MmiGotoxy(0,i1++); MmiPrintf("Консоль не в раб.позиции ");
      ...
      ///yyy//////////////////////////////////////////
      case CN_THR_E:
MmiGotoxy(0,i1++); MmiPrintf("Консоль не в горловине   ");
        if(i1 >= n_mmi_str) return;
        break;
      ////////////////////////////////////////////////        
     }
...
///yyy//////////////////////////////////
"Сгн.'ГОРЛОВ.' N вх.",      // 166
"Сгн.'ГОРЛОВ.'фл.инв",      // 167
////////////////////////////////////////
//04.02.2020 YN  -----\\//-----
"Задв.В.Закр. N вх.",       // 168
"Задв.В.Закр.фл.инв",       // 169
"Задв.Н.Закр. N вх.",       // 170
"Задв.Н.Закр.фл.инв",       // 171
"Давл.415М:Статус",         // 172
"Давл.АИР20:Статус",        // 173
"Темп.ТПУ :Статус",         // 174
"Отс.клапан N вх. ",        // 175
"Отс.клапан фл.инв",        // 176
//04.02.2020 YN  -----//\\-----
"",
};
//-------------------------------
          ...
          if(PageD==0)
            {
               ...
               MmiGotoxy(0,12);  MmiPrintf("Давление 1  :%7.4f МПа    ",s_MVD[0].Press);
               //27.01.20 YN  -----\\//-----
               MmiGotoxy(0,13);  MmiPrintf("Давление 2  :%7.4f МПа    ", Press_N2);
               //27.01.20 YN  -----//\\-----
...
//26.02.2020 YN  -----\\//-----
       else if(key== '0' )// 0  Проверка приведения плотности
       {
         return 1;
       }
      /* {
m_m0s:
         SetDisplayPage(EmptPage);
         f_clr_scr_MMI();
            MmiGotoxy(0,0);    MmiPrintf("АСН %d. Пров-ка.привед.плотности.",n_ASN);
            MmiGotoxy(0,2);    MmiPuts("1 Текущая плотность");
            MmiGotoxy(0,3);    MmiPuts("2 Текущая температура");
            MmiGotoxy(0,4);    MmiPuts("3 Текущее давление");
            MmiGotoxy(0,5);    MmiPuts("4 Отпущенная масса");
            //MoveToXY(0,4);   MmiPrintf("Множитель массы:     %.6f",s_MVD[0].f);
            //MoveToXY(0,5);   MmiPrintf("Множитель объема:    %.6f",s_MVD[0].f);
            dens_nm_15= f_get_ref_Dns(dens_nm, temp_nm , press_nm, 15.);
            vol_nm_15= mass_nm/dens_nm_15;
            MmiGotoxy(0,7);    MmiPrintf("Тек. плотность: %3.3f кг/м3",dens_nm);
            MmiGotoxy(0,8);    MmiPrintf("Тек. темпер-ра: %4.2f   С",temp_nm);
            MmiGotoxy(0,9);    MmiPrintf("Тек. давление : %3.3f   Мпа",press_nm);
            MmiGotoxy(0,10);   MmiPrintf("Отпущен. масса: %3.2f кг",mass_nm);
            MmiGotoxy(0,12);   MmiPrintf("Плотность 15С : %3.3f кг/м3",dens_nm_15);
            MmiGotoxy(0,13);   MmiPrintf("Объем при 15С : %3.3f м3",vol_nm_15);
...
//26.02.2020 YN  -----\\//-----
 /*========================================*/
 /*   case 128:
 //  Меню 0S. Проверка приведения плотности
       if((key==ESC)||(key==ENTER) )   */ /* переход в меню */
     /*  {
         goto main_menu;
       }
...
m_m0s_1:
          ...
          SetDisplayPage(EmptPage); // ввод значения
          f_clr_scr_MMI();
          MmiPuts("Введите плотность кг/м3: " );
          k_mv_tmp=dens_nm;
          flag_mass=1;
m_m0s_1_1:
...
m_m0s_4:
       ...
       else if(key==DATA)
        {
         ...
         if(flag_mass==1)
         {
          if(type_liq==2)
          {
            if((k_mv_tmp<800)||(k_mv_tmp>900.) )
                goto m_m0s_1;
          }
          else if (type_liq==5) 
          {
            if((k_mv_tmp<700)||(k_mv_tmp>800.) )
                goto m_m0s_1;
          }
         }
         else if (flag_mass==0)
         {
          if((k_mv_tmp<0)||(k_mv_tmp>100.) )
                goto m_m0s_2;
         }
         else if (flag_mass==2)
         {
           if((k_mv_tmp<0)||(k_mv_tmp>1.) )
                goto m_m0s_2;
         }
         else if (flag_mass==3)
         {
            if((k_mv_tmp<0)||(k_mv_tmp>10000) )
                goto m_m0s_3;
         }
